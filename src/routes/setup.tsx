    ...
    try {
      ...
      if (data.user) {
        if (!data.session) {
          ...
          const { error: upsertError } = await supabase.from("alert_preferences").upsert({
            user_id: data.user.id,
            threshold,
            notify_slack: true,
            notify_email: true,
          });
          if (upsertError) console.error("Error setting alert preferences:", upsertError);
        }
                ...
                <div className="mt-auto grid grid-cols-2 gap-3 bg-black/40 p-4 border border-[#343940]/45">
                  {[
                    ...
                    return (
                      <button
                        key={src.key}
                        onClick={() => toggleSource(src.key as keyof typeof connectedSources)}
                        type="button"
